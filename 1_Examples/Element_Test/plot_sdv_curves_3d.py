...
def _get_val(row, key):
    ...
    if key.startswith("SDV"):
        alt = "L" + key
        if alt in row:
            return _to_float(row.get(alt))
    return _to_float(row.get(key))
...
def main():
    ...
    parser.add_argument("--normal_z", default="SDV7")
    parser.add_argument("--shear_y", default="SDV4")
    parser.add_argument("--shear_z", default="SDV6")
    ...
    tz = []
    for row in rows:
        dx = _get_val(row, args.normal_x)
        dy = _get_val(row, args.shear_y)
        tn = _get_val(row, args.normal_z)
        ts = _get_val(row, args.shear_z)
    ...
    if not args.mixed_surface and (args.data_points or args.pure_only):
        ax.scatter(tx, ty, tz, color=MUL_COLORS["warm"], s=12, alpha=0.8, label="Resultant (data)")
